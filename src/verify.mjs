#!/usr/bin/env node
// verify.mjs — State Employment AI Disclosure Tracker verifier.
...
const LEGAL = {
  "proposed":                  new Set(["in-committee", "passed-one-chamber", "enacted-awaiting-effective", "withdrawn", "vetoed"]),
  "in-committee":              new Set(["passed-one-chamber", "withdrawn"]),
  "passed-one-chamber":        new Set(["enacted-awaiting-effective", "withdrawn", "vetoed"]),
...
function main() {
  ...
  if (files.length === 0) { console.log("OK — 0 jurisdiction streams."); return; }
  ...
  console.log(`OK — ${total} events across ${files.length} jurisdiction stream(s) validated, transitions legal.`);
}
